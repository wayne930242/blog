...
- $0$：數字 $0$ 屬於概念 $F$，當且僅當，命題「無論 $a$ 為何，$a$ 不屬於 $F$」普遍為真時。
- $1$：數字 $1$ 屬於概念 $F$，當且僅當，命題「存在 $a$ 使得 $a$ 屬於 $F$」普遍為真，並且若命題「$a$ 屬於 $F$」且「$b$ 屬於 $F$」，則普遍地 $a$ 和 $b$ 相同。
- $n+1$：數字 $n+1$ 屬於概念 $F$，當且僅當，命題「存在 $a$ 使得 $a$ 屬於 $F$」普遍為真，並且數字 $n$ 屬於概念「屬於 $F$，但不是 $a$」。

Frege 指出這組定義目前的問題在於，我們不知道一般而言的 $n$ 屬於 $F$ 的 $n$ 是否是數字，這麼一來我們也無從確定適用於它們的等同性。同樣地，我們目前也只知道 $0$ 和 $1$ 可以被定義，還不確定它們就是同樣的自存對象，即都是數字。

回顧上節的結論：**數字陳述的內容是關於概念的斷言**。Frege 要我們注意，他避免去說數字是概念的性質，因為數字只是在斷言內容中形成元素，個別的數字就本身而言是自存的對象（it forms only an element in what is asserted, the individual number shows itself for what it is, a self-subsistent object.）。Frege 表明，當我們說「木星有四個衛星」，也可以說「木星的衛星數是四」，而這裡的「是」的含義是等同或等於的意涵，更進一步說，「木星的衛星數」和「四」是同一個對象。
...
Frege 打算先定義的是「『屬於概念 $F$ 的數字』與『屬於概念 $G$ 的數字』」相同的意涵。他的策略是透過已知的等同性來定義數值等同性，讓數值等同判斷的「等號」兩側都會是數字。他首先定義：

- (1) 屬於概念 $F$ 的數字是概念「等於概念 $F$」的外延。

Frege 用平行線和相似形來舉例：線段 $a$ 的方向是概念「平行於線段 $a$」的外延，而三角形 $t$ 的形狀是概念「相似於三角形 $t$」的外延。這種定義的方式，我認為相當接近現代的等同類（equuivalence class）的定義方式。

首先，Frege 這樣定義概念的「等於」：

- (2) 「概念 $F$ 等於概念 $G$」，若且唯若，存在一個關係 $\phi$ 使得屬於概念 $F$ 的對象和屬於 $G$ 的對象能相互一對一地關聯起來。
...
- (4) 「等於概念 $F$」之概念的外延等於「等於概念 $G$」之概念的外延，若且唯若，屬於概念 $F$ 的數字，也會屬於概念 $G$。

Frege 有意識到需要檢查這個「等於」概念的遞移律，而他的「一對一對應」其實也已經蘊含了交換律。現代的的標準作法是會檢查這個等於關係是一個等同關係，Frege 的作法已經很類似。

接著 Frege 這樣定義 $0$：

- (5) $0$ 是屬於概念「自身不等同（identical with）自身」的數字。

接著 Frege 定義自然數中的相鄰數：

- (6) $n^+$ 跟隨在自然數序列中的 $n$，若且唯若，存在概念 $F$ 及 $F$ 外延中的對象 $x$，使得屬於概念「在 $F$ 中但不等同於 $x$」的數字是 $n$，並且屬於 $F$ 概念的數字是 $n^+$。

透過 $0$，Frege 這樣定義 $1$：

- (7) $1$ 是屬於概念「與 $0$ 等同（的數字）」的數字。

由於 $1$ 裡面有 $0$ 這個對象，並且除去這個對象，$1$ 就等於 $0$ 了，這就說明了 $1$ 跟 $0$ 是相鄰的。

接著 Frege 提出了五個（他覺得）可以簡單證明的命題，分別是：

1. 如果 $a$ 在自然數序列中跟隨著 $0$，則 $a$ 等於 $1$。
2. 如果 $1$ 是屬於一概念的數字，則存在一個對象在該概念中。
3. 如果 $1$ 是屬於概念 $F$ 的數字，則有對象（稱其為 $x$）在概念 $F$ 中，且若 $y$ 也在概念 $F$ 中，$x$ 等同 $y$。
4. 如果一個對象在概念 $F$ 中，且可以從「$x$ 在 $F$ 概念中」和「$y$ 在 $F$ 概念中」推斷出 $x$ 等同於 $y$，則 $1$ 是屬於概念 $F$ 的數字。
5. 由命題「$n^+$ 在自然數序列中跟隨 $n$」建立的 $m$ 與 $n$ 是一對一對應關係。

接著 $Frege$ 要證明對於每一個數字 $n$ 都有 $n^+$ 在其後。為此，他選擇的概念是「結尾為 $n$ 的自然數序列成員」，這來自他的 _Begriffsschrift_ ：

- (8) $x^+$ 在 $\phi$-序列中緊跟隨 $x$，或說 $x$ 在 $\phi$-序列中緊先於 $x^+$，若且唯若，若每個和 $x$ 在關係 $\phi$ 中的對象都在 $F$ 概念中並且能從「$d$ 在概念 $F$ 中」普遍推出「無論 $d$ 為何，每個和 $d$ 在關係 $\phi$ 中的對象都在 $F$ 概念中」，則 $x^+$ 在概念 $F$ 中。
...
- $n^+ = \langle 0, 1, 2, 3, \ldots, n \rangle$
