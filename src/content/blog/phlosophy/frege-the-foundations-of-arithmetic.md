...
Frege 打算先定義的是「『屬於概念 $F$ 的數字』與『屬於概念 $G$ 的數字』」相同的意涵。他的策略是透過已知的等同性來定義數值等同性，讓數值等同判斷的「等號」兩側都會是數字。他首先定義：屬於概念 $F$ 的數字是概念「等於概念 $F$」的外延。

Frege 用平行線和相似形來舉例：線段 $a$ 的方向是概念「平行於線段 $a$」的外延，而三角形 $t$ 的形狀是概念「相似於三角形 $t$」的外延。

Frege 指出這是正確的：命題「『等同於概念 $F$』概念的外延等同於『等同於概念 $G$』概念的外延」成立，若且唯若命題「相同的數字屬於概念 $F$，也會屬於概念 $G$」成立。
